...
class EditableCell extends React.Component {
    ...
    getInput = () => {
        ...
        if (this.props.inputType === 'double') {
            return <InputNumber precision={2}/>;
        }
        if (this.props.inputType === 'bigText') {
            return <TextArea style={{ height: 100 }}/>;
        }
...
class GoodsManager extends React.Component {
    constructor(props) {
        ...
        this.columns = [
            ...
            { title: '类别名', dataIndex: 'categoryName', width: '8.5%', editable: true, inputType: 'category' },
            {
                ...
                width: '10%',
                render: text => <img style={{ height: 100, wdith: 170 }} alt='' src={text}/>,
            },
    ...
    keywordChange = (e) => {
        this.setState({ goodsListQO: { ...this.state.goodsListQO, keyword: e.target.value } });
    };
    qoCategoryChange = (value) => {
        this.setState({ goodsListQO: { ...this.state.goodsListQO, category: value } });
    };
    qoSaleStatusChange = (value) => {
        this.setState({ goodsListQO: { ...this.state.goodsListQO, onSale: value } });
    };
    ...
    partUpdateGoods(goodsBO) {
        goodsBO = {
            ...
            categoryId: categoryList.find(function(x) {
                return x.categoryName === goodsBO.categoryName;
            }).id,
            price: goodsBO.price,
    ...
    addGoodsNames = (e) => {
        this.setState({ newGoods: { ...this.state.newGoods, goodsName: e.target.value } });
    };
    addGoodsCategory = (value) => {
        this.setState({ newGoods: { ...this.state.newGoods, categoryId: value } });
    };
    addGoodsIntroduction = (e) => {
        this.setState({ newGoods: { ...this.state.newGoods, introduction: e.target.value } });
    };
    addGoodsPrice = (value) => {
        this.setState({ newGoods: { ...this.state.newGoods, price: value } });
    };
    addGoodsSaleStatus = (value) => {
        this.setState({ newGoods: { ...this.state.newGoods, onSale: value } });
    };
    addGoodsQuantity = (value) => {
        console.log(value);
        this.setState({ newGoods: { ...this.state.newGoods, quantity: value } });
    };
    //添加商品modal的主图upload
                    ...
                    <div style={{ padding: 24 }}>
                        ...
                        <EditableContext.Provider value={this.props.form}>
                            <div style={{ paddingBottom: 24 }}>
                                <Input style={{ width: 200 }} placeholder="商品名" allowClear
                                       onChange={this.keywordChange}/>&nbsp;&nbsp;&nbsp;
                                <Select style={{ width: 100 }} placeholder='商品类别' allowClear
                                        onChange={this.qoCategoryChange}>
                                    {categoryList.map(function(data) {
                                        return <Option value={data.id} key={data.id}>{data.categoryName}</Option>;
                                    })}
                                </Select>&nbsp;&nbsp;&nbsp;
                                <Select style={{ width: 100 }} placeholder='上架状态' allowClear
                                        onChange={this.qoSaleStatusChange}>
                                    <Option value="1" key={1}>在售</Option>
                                    <Option value="0" key={0}>未上架</Option>
                                </Select>&nbsp;&nbsp;&nbsp;
                                <Button type='primary'
                                        onClick={() => this.changeGoodsList()}>搜索</Button>&nbsp;&nbsp;&nbsp;
                            </div>
                            <Table
